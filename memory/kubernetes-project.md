...
| `POST /deploys` → `500`, log shows `PrismaClientKnownRequestError: table "public.deploys" does not exist` | No `prisma/migrations/` in either `deploy-service` or `log-service` — schema was likely only ever pushed once by hand into the old Compose Postgres volume, which K8s's brand-new PVC doesn't have. Production image also has no `prisma` CLI (`npm ci --omit=dev` drops the devDependency) | Ran schema push from host, not from inside the pod: `kubectl port-forward svc/postgres 5432:5432 -n deploy-status-checker`, then `DATABASE_URL=postgresql://deploy_user:<pw>@localhost:5432/deploy_db npx prisma db push` (repeat for `log_db`/`log_user`) using the local `node_modules/.bin/prisma`. **Standing gap, not just a K8s quirk** — this project has no repeatable migration step at all (no `prisma migrate`, no CI/entrypoint step); anyone rebuilding the DB from scratch hits this |
